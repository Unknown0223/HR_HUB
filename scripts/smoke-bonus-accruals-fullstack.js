/**
 * Бонусные начисления — API persist + Chrome list/new.
 * Usage: node scripts/smoke-bonus-accruals-fullstack.js
 */
...
async function launchBrowser() {
  ...
  if (!bin) throw new Error('Chrome/Edge topilmadi — CHROME_PATH ni belgilang');
    const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hrhub-bonus-'));
...
async function openPage(cdp, sessionId, href) {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    if (!last.stillLoading && /(создать|сохранить)/i.test(fold(last.body))) break;
  }
  return last;
}
...
async function main() {
  ...
  console.log('—— 1) API smoke:bonus-accruals ——');
  const apiSmoke = spawnSync(process.execPath, [path.join('scripts', 'smoke-bonus-accruals.js')], {
    cwd: root,
    encoding: 'utf8',
    env: process.env,
  });
  ...
  try {
    ...
    await check('list Бонусные начисления', async () => {
      const p = await openPage(cdp, sessionId, '/catalog/bonus-accruals');
    ...
    await check('new Факт', async () => {
      const p = await openPage(cdp, sessionId, '/catalog/bonus-accruals/new?kind=fact');
      assert(!p.hasServerError, p.body.slice(0, 400));
      mustHave(
        p.body,
        [
          'Бонусное начисление - Факт (создание)',
          'Сохранить',
          'Провести',
          'Закрыть',
          'Дата',
    ...
    await check('new КПЭ', async () => {
      const p = await openPage(cdp, sessionId, '/catalog/bonus-accruals/new?kind=kpi');
      assert(!p.hasServerError, p.body.slice(0, 400));
      mustHave(
        p.body,
        [
          'Бонусные начисления - КПЭ (создание)',
          'Сохранить',
          'Провести',
          'Закрыть',
          'Подразделение',
    ...
    await check('regression Авансовый отчет', async () => {
      const p = await openPage(cdp, sessionId, '/catalog/travel-expenses');
      assert(!p.hasServerError, p.body.slice(0, 300));
      mustHave(p.body, ['Авансовый отчет по командировке'], 'travel-expenses');
    });
  } finally {
  ...
  console.log('✓ bonus-accruals fullstack + regression ok');
}
