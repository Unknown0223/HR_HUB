/**
 * Разовые начисления — API persist + Chrome list/new.
...
async function launchBrowser() {
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hrhub-ot-'));
...
async function main() {
  ...
  console.log('—— 1) API smoke:one-time-accruals ——');
  const apiSmoke = spawnSync(process.execPath, [path.join('scripts', 'smoke-one-time-accruals.js')], {
    cwd: root,
    encoding: 'utf8',
    env: process.env,
  });
  ...
  try {
    ...
    await check('list Разовые начисления', async () => {
      const p = await openPage(cdp, sessionId, '/catalog/one-time-accruals');
      assert(!p.hasServerError, p.body.slice(0, 400));
      mustHave(
        p.body,
        ['Разовые начисления', 'Начисление', 'Удержание', 'Создать', 'Месяц', 'Дата', 'Номер', 'Валюта', 'Название документа', 'Проведен', 'Нет данных'],
        'list',
      );
    });

    await check('new Разовое начисление', async () => {
      const p = await openPage(cdp, sessionId, '/catalog/one-time-accruals/new?kind=accrual');
      assert(!p.hasServerError, p.body.slice(0, 400));
      mustHave(
        p.body,
        [
          'Разовое начисление (создание)',
          'Сохранить',
          'Провести',
          'Закрыть',
          'Основание',
          'Тип расчета',
          'По введенному значению',
          'В процентах',
          'Формула',
          'Использовать одно начисление ко всем',
          'Подбор',
          'Подбор инициаторов',
          'Импорт',
          'Сотрудник',
    ...
    await check('tab Удержание', async () => {
      const p = await openPage(cdp, sessionId, '/catalog/one-time-accruals?kind=deduction');
      assert(!p.hasServerError, p.body.slice(0, 400));
      mustHave(p.body, ['Разовые начисления', 'Удержание', 'Нет данных'], 'deduction');
    });

    await check('regression Начисления процентов от продаж', async () => {
      const p = await openPage(cdp, sessionId, '/catalog/sales-accruals');
      assert(!p.hasServerError, p.body.slice(0, 300));
      mustHave(p.body, ['Начисления процентов от продаж'], 'sales');
    });
  } finally {
  ...
  console.log('✓ one-time-accruals fullstack + regression ok');
}
