"""Pack HR HUB JSON (unmodified copies) + Postgres snapshot for a new machine.

Does not rewrite dump rows. Credentials are never copied.

  python scripts/catalog-pack-transfer.py
  python scripts/hrhub-restore-machine.py
"""
...
LIVE = ROOT / "data" / "hrhub-dump" / "live"
PACK = ROOT / "data" / "hrhub-dump" / "transfer"
DATA_NAMES = [
    "employees",
    "divisions",
    "jobs",
...
def pack():
    ...
    manifest = {
        ...
        "filial": "88862",
        "note": "Raw HR HUB JSON copies. Rows are not rewritten.",
        "files": {},
    }
    missing = []
...
def restore():
    ...
    if not src_live.exists():
        raise SystemExit(f"Paket yo‘q: {src_live} — avval catalog-pack-transfer.py")
    ...
    api = ROOT / "apps" / "api"
    r = subprocess.run(["node", "scripts/import-catalog-live.js"], cwd=str(api))
    if r.returncode != 0:
        raise SystemExit(r.returncode)
    r = subprocess.run(["node", "scripts/import-catalog-live-extra.js"], cwd=str(api))
    if r.returncode != 0:
        raise SystemExit(r.returncode)
    print("RESTORE DONE")
