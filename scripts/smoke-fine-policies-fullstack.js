/**
 * Fine Policies — API persist + authenticated Chrome pages (list/tabs/form/modal/redirect).
 * Usage: node scripts/smoke-fine-policies-fullstack.js
...
async function main() {
  ...
  console.log('—— 1) API smoke:fine-policies ——');
  const apiSmoke = spawnSync(process.execPath, [path.join('scripts', 'smoke-fine-policies.js')], {
    cwd: root,
    encoding: 'utf8',
    env: process.env,
  });
  ...
  assert(division && position && employee, 'lookups yetarli emas');

  const leftovers = await req('GET', '/api/payroll/fine-policies', auth);
  const leftoverIds = (leftovers.data || [])
    .filter((r) => /^(UI_|SMOKE_FP_|DIV_)/.test(r.name || ''))
    .map((r) => r.id);
  if (leftoverIds.length) {
    await req('POST', '/api/payroll/fine-policies/bulk-delete', {
      ...auth,
  ...
  const created = await req('POST', '/api/payroll/fine-policies', {
    ...
    body: {
      ...
      name: `UI_${stamp}`,
      rules: {
        late: [{ type: 'coefficient', value: 2, timeFrom: 1, timeTo: 10 }],
      },
    },
  });
  ...
  try {
    ...
    await check('list /payroll/fine-policies', async () => {
      const p = await openPage(cdp, sessionId, '/payroll/fine-policies');
    ...
    await check('tabs division/position/employee', async () => {
      for (const [tab, extra] of [
        ['division', 'Подразделение'],
        ['position', 'Должность'],
        ['employee', 'Сотрудники'],
      ]) {
        const p = await openPage(cdp, sessionId, `/payroll/fine-policies?tab=${tab}`);
        mustHave(p.body, ['Политики штрафов', 'Создать', extra], tab);
        assert(p.href.includes(`tab=${tab}`), `url tab=${tab} ${p.href}`);
      }
    });

    await check('create form company', async () => {
      const p = await openPage(cdp, sessionId, '/payroll/fine-policies/new');
      mustHave(p.body, [
        'Политика штрафов (создание)',
        'Сохранить',
        'Закрыть',
        'Месяц',
        'Название',
    ...
    await check('create form division/position/employee fields', async () => {
      const d = await openPage(cdp, sessionId, '/payroll/fine-policies/new?tab=division');
      mustHave(d.body, ['Подразделение'], 'div form');
      const pos = await openPage(cdp, sessionId, '/payroll/fine-policies/new?tab=position');
      mustHave(pos.body, ['Должность'], 'pos form');
      const emp = await openPage(cdp, sessionId, '/payroll/fine-policies/new?tab=employee');
      mustHave(emp.body, ['Сотрудники'], 'emp form');
    });

    await check('edit form + rule modal', async () => {
      const p = await openPage(cdp, sessionId, `/payroll/fine-policies/${id}/edit`);
      mustHave(p.body, ['Политика штрафов (изменение)', 'Коэффициент'], 'edit');
      const nameVal = await evaluate(
        cdp,
        sessionId,
        `[...document.querySelectorAll('input')].map((i) => i.value).join(' ')`,
      );
      assert(fold(nameVal).includes(fold(`UI_${stamp}`)), `name input ${nameVal}`);
      ...
      assert(modal.ok, modal.why || 'modal open');
      await sleep(400);
      const after = await evaluate(cdp, sessionId, `document.body.innerText`);
      mustHave(after, ['Опоздание', 'Коэффициент', 'Сумма', 'Время', 'Аннулирование', 'Процент от заработной платы', 'Значение'], 'modal');
    });

    await check('redirect /payroll?tab=policies → fine-policies', async () => {
      const p = await openPage(cdp, sessionId, '/payroll?tab=policies');
      await sleep(800);
      const href = await evaluate(cdp, sessionId, `location.pathname + location.search`);
      assert(
        href.startsWith('/payroll/fine-policies') || p.body.includes('Политики штрафов'),
        `redirect emas: ${href}`,
      );
    });

    await check('allowance stub still on payroll', async () => {
      const p = await openPage(cdp, sessionId, '/payroll?tab=policies&kind=allowance');
      mustHave(p.body, ['Политика доплат'], 'allowance');
      assert(p.href.includes('kind=allowance') || p.body.includes('Доплата'), p.href);
    });

    await check('row actions Изменить / Удалить / Скопировать', async () => {
      await openPage(cdp, sessionId, '/payroll/fine-policies');
  ...
  console.log('✓ fine-policies fullstack: API + browser pages ready');
}
