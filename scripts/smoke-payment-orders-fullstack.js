/**
 * Поручения — API persist + Chrome list/new.
...
async function launchBrowser() {
  ...
  if (!bin) throw new Error('Chrome/Edge topilmadi — CHROME_PATH ni belgilang');
    const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hrhub-po-'));
...
async function main() {
  ...
  console.log('—— 1) API smoke:payment-orders ——');
  const apiSmoke = spawnSync(process.execPath, [path.join('scripts', 'smoke-payment-orders.js')], {
    cwd: root,
    encoding: 'utf8',
    env: process.env,
  });
  ...
  try {
    ...
    await check('list Поручения', async () => {
    ...
    await check('new Поручение', async () => {
      const p = await openPage(cdp, sessionId, '/catalog/payment-orders/new');
      assert(!p.hasServerError, p.body.slice(0, 400));
      mustHave(
        p.body,
        [
  ...
  console.log('✓ payment-orders fullstack + regression ok');
}
