/**
 * Авансовый отчет по командировке — API persist + Chrome list/new.
 * Usage: node scripts/smoke-travel-expenses-fullstack.js
 */
...
async function launchBrowser() {
  ...
  if (!bin) throw new Error('Chrome/Edge topilmadi — CHROME_PATH ni belgilang');
    const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hrhub-travel-'));
...
async function main() {
  ...
  console.log('—— 1) API smoke:travel-expenses ——');
  const apiSmoke = spawnSync(process.execPath, [path.join('scripts', 'smoke-travel-expenses.js')], {
    cwd: root,
    encoding: 'utf8',
    env: process.env,
  });
  ...
  try {
    ...
    await check('list Авансовый отчет', async () => {
    ...
    await check('new Авансовый отчет', async () => {
      const p = await openPage(cdp, sessionId, '/catalog/travel-expenses/new');
      assert(!p.hasServerError, p.body.slice(0, 400));
      mustHave(
        p.body,
        [
          'Авансовый отчет по командировке (создание)',
          'Сохранить',
          'Завершить',
          'Закрыть',
          'Номер',
    ...
    await check('regression Поручения', async () => {
      const p = await openPage(cdp, sessionId, '/catalog/payment-orders');
      assert(!p.hasServerError, p.body.slice(0, 300));
      mustHave(p.body, ['Поручения'], 'payment-orders');
    });
  } finally {
  ...
  console.log('✓ travel-expenses fullstack + regression ok');
}
