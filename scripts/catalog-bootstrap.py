"""Download HR HUB live data and import it into this HR Hub instance.

Needs:
  data/catalog-dump/live/credentials.json  {"login":"...","password":"..."}
  or env HR_HUB_LOGIN / HR_HUB_PASSWORD

  Local Postgres (npm run infra:up) and seeded tenant (npm run db:seed).

Usage:
  python scripts/catalog-bootstrap.py
  python scripts/catalog-bootstrap.py --dump-only
  python scripts/catalog-bootstrap.py --import-only
  python scripts/catalog-bootstrap.py --skip-tracks
"""
...
def main():
    p = argparse.ArgumentParser(description="HR HUB -> HR Hub bootstrap")
    p.add_argument("--dump-only", action="store_true", help="Faqat katalogdan yuklab olish")
    ...
    if not args.import_only and not cred.exists():
        print(
            ...
            f"  {cred}\n"
            '  {"login":"...","password":"..."}\n'
            "yoki HR_HUB_LOGIN / HR_HUB_PASSWORD qo‘ying."
        )
        raise SystemExit(2)
    ...
    run_node("scripts/import-catalog-live.js")
    run_node("scripts/import-catalog-live-extra.js")
