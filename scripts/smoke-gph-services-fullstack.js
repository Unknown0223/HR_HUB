/**
 * Список услуг договора ГПХ — API persist + authenticated Chrome pages.
 * Usage: node scripts/smoke-gph-services-fullstack.js
 */
...
async function launchBrowser() {
  ...
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hrhub-gphs-'));
...
async function main() {
  ...
  console.log('—— 1) API smoke:gph-services ——');
  const apiSmoke = spawnSync(process.execPath, [path.join('scripts', 'smoke-gph-services.js')], {
    cwd: root,
    encoding: 'utf8',
    env: process.env,
  });
  ...
  pass('API persist + contract bog‘lanishi');
  ...
  const createdC = await req('POST', '/api/catalog/gph-contracts', {
    ...
    body: {
      employeeId: emp.id,
      number: `SMOKE-GPH-C-UI-${stamp}`,
      title: 'SMOKE GPH UI',
      startDate: '2026-08-01',
      allowAddService: true,
    },
  });
  assert(createdC.ok, `contract ${createdC.status} ${JSON.stringify(createdC.data)}`);
  const contractId = createdC.data.id;
  const contractNumber = createdC.data.number;

  const seeded = await req('POST', '/api/catalog/gph-services', {
    ...auth,
    body: {
      code: `SMOKE_GPH_UI_${stamp}`,
      name: 'SMOKE_GPH_UI',
      contractId,
      month: '2026-08-01',
      status: 'draft',
  ...
  try {
    ...
    await check('list Список услуг договора ГПХ', async () => {
      const p = await openPage(cdp, sessionId, '/catalog/gph-services');
      assert(!p.stillLoading && !p.hasServerError, p.body.slice(0, 400));
      mustHave(
        p.body,
        [
          'Список услуг договора ГПХ',
          'Номер договора',
          'Месяц',
          'Подразделение',
          'Физическое лицо',
    ...
    await check('row actions', async () => {
      await openPage(cdp, sessionId, '/catalog/gph-services');
      const opened = await evaluate(
        cdp,
        sessionId,
        `(() => {
          const cell = [...document.querySelectorAll('td')].find((td) =>
            (td.textContent || '').includes(${JSON.stringify(contractNumber)}),
          );
      ...
      mustHave(body, ['Изменить', 'Удалить'], 'row actions');
    });
    ...
    await check('regression Договор ГПХ', async () => {
      ...
      mustHave(p.body, ['Договор ГПХ', 'Номер договора'], 'gph-contracts');
    });
    ...
    await check('regression Ручные операции', async () => {
      const p = await openPage(cdp, sessionId, '/payroll/manual');
      assert(!p.hasServerError, p.body.slice(0, 300));
      mustHave(p.body, ['Ручные операции'], 'manual');
    });
  } finally {
    if (browser) await browser.close();
    if (docId) await req('DELETE', `/api/catalog/gph-services/${docId}`, auth);
    if (contractId) await req('DELETE', `/api/catalog/gph-contracts/${contractId}`, auth);
  }

  console.log(`\n${passed.length} passed, ${failed.length} failed`);
  if (failed.length) throw new Error(failed.join('\n'));
  console.log('✓ gph-services fullstack + regression ok');
}
