# HR HUB — HR HUB Backend 1:1 Plan

**Goal:** Backend/DB/API/business-logic parity with HR HUB catalog workflows (approve/post, attendance→payroll FOT chain).  
**Not the goal:** Frontend pixel-clone of HR HUB Excel UI.
...
| Area | In scope |
|------|----------|
| **Entities** | Same business objects HR HUB lists (employees, absences, HR docs, name/wage changes, clearance, timesheet, periods, advances, accruals, FOT inputs) |
| **Statuses** | Explicit lifecycle enums (draft / pending / approved / rejected / posted / closed / …) matching HR HUB document semantics |
...
| Create-noise HR HUB screens excluded from mega-nav | Already filtered |
| Git commit unless asked | User rule |

---

## 2. Inventory — HR HUB workflows → HR HUB gaps

> **Eslatma (2026-09):** quyidagi jadval — tarixiy gap inventar. §3 dagi P0 checkboxlar (`[x]`) joriy bajarilish holatini bildiradi. Inventar qatorlari hali «PARTIAL» deb qolishi mumkin; yangi ish uchun §3 + [IMPROVEMENT_MASTER_PLAN.md](./IMPROVEMENT_MASTER_PLAN.md) F4 ni kuzating.
...
| HR HUB module | HR HUB today | Gap |
|----------------|--------------|-----|
...
- [x] Incident resolve/close helpers (if not via catalog patch)
...
### Phase A — Plan
- [x] This file created (`CATALOG_BACKEND_PARITY_PLAN.md`)
...
| Date | Note |
|------|------|
| 2026-07-25 | Plan authored from HR HUB catalog + current Prisma/API audit; Phase B started |
