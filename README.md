...
**Holat:** Phase 0–6 navbar MVP ✅ · yaxshilash fazalari: F0–F6.  
Master-reja: [`docs/IMPROVEMENT_MASTER_PLAN.md`](./docs/IMPROVEMENT_MASTER_PLAN.md) · Status: [`docs/STATUS.md`](./docs/STATUS.md) · Security: [`docs/SECURITY_CHECKLIST.md`](./docs/SECURITY_CHECKLIST.md) · Smoke: [`docs/SMOKE_INDEX.md`](./docs/SMOKE_INDEX.md)
...
npm run dev:gw           # :8800 (yoki: cd apps/device-gw && .venv\Scripts\python -m uvicorn main:app --reload --port 8800)
```
...
| API Swagger | http://localhost:3002/docs |
| Device GW | http://localhost:8800/docs |
...
| `NEXT_PUBLIC_API_URL` | Web | Default `http://localhost:3002` |
...
```bash
curl -X POST http://localhost:3002/api/attendance/punches/ingest \
...
npm run load:punch:heavy              # 500 / 80 + --write-results
# yoki
node scripts/load-punch-test.js --n 500 --concurrency 80 --write-results
```

Muvaffaqiyat / latency (avg, p50, p95, p99) chiqadi. `PUNCH_INGEST_API_KEY` o‘rnatilgan bo‘lsa, skript `X-Punch-Key` yuboradi. Heavy profil `--write-results` bilan natijani `docs/` ga yozishi mumkin.
...
- Retention: `FACE_PURGE_DAYS` (Nest cron 03:00) yoki `npm run face:purge` — soft-clear MinIO + FaceProfile; audit `face.retention_purge`. Batafsil: [`docs/SECURITY_CHECKLIST.md`](./docs/SECURITY_CHECKLIST.md), [`docs/DEPLOY.md`](./docs/DEPLOY.md).
...
Adapters: `GET http://127.0.0.1:8800/adapters` → `mock`, `hikvision_isapi`, `zkteco_push`.

### Mock

```bash
curl -X POST "http://localhost:8800/devices/<NEST_DEVICE_UUID>/emit-mock-punch?employee_external_id=face-0001&direction=IN"
```
