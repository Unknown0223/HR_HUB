/**
 * Import gathered HR HUB dump (data/catalog-dump/*.json) into HR Hub.
 *
...
const DUMP = path.join(ROOT, 'data', 'hrhub-dump');
...
(async () => {
  if (!fs.existsSync(path.join(DUMP, 'employees.json'))) {
    throw new Error('Dump yo‘q. Avval: python scripts/catalog-gather.py');
  }
    ...
    if (found) {
      await prisma.workSchedule.update({
        ...
        data: {
          ...
          endTime: meta.end,
          settings: { weekPattern: meta.pat, dayNormHours: 8, source: 'hrhub' },
          isActive: true,
        },
      });
  ...
  const hired = await prisma.employee.findMany({
    where: { tenantId: tenant.id, externalId: { startsWith: 'hrhub:' } },
  ...
  for (const emp of hired) {
    ...
    hireCreates.push({
      ...
      postedBy: 'hrhub-import',
      payload: { divisionId: emp.divisionId, positionId: emp.positionId, source: 'hrhub' },
    });
  }
  ...
  await prisma.candidate.deleteMany({
    where: { tenantId: tenant.id, note: 'hrhub-import' },
  });
  let candN = 0;
  const types = ['A', 'B', 'C'];
  ...
  if (schedDoc) {
    ...
    schedDoc = await prisma.individualSchedule.create({
      data: {
        ...
        note: 'HR HUB план графиков август 2026',
        verified: true,
        postedAt: new Date(),
        postedBy: 'hrhub-import',
        settings: { source: 'hrhub', interval: 'month' },
      },
    });
  }
