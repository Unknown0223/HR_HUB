/**
 * Import live HR HUB dump (data/catalog-dump/live/*.json) into local HR Hub.
 *
 * Matches Excel-imported employees by staff/robot code (externalId hrhub:{robot_id}
...
const DUMP = path.join(ROOT, 'data', 'hrhub-dump', 'live');
...
(async () => {
  ...
  for (const name of scheduleNames) {
    ...
    schedByName[name] = await prisma.workSchedule.create({
      data: {
        ...
        endTime: meta.end,
        settings: { weekPattern: meta.pat, dayNormHours: 8, source: 'hrhub-live' },
      },
    });
  }
  ...
  function indexEmp(emp, r, _robotId, fio) {
    ...
    if (r?.employee_id) empByExt.set(`hrhub:${r.employee_id}`, emp);
  }
  ...
  for (let i = 0; i < employeesDump.length; i += 1) {
    ...
    let emp = empByExt.get(`hrhub:${r.employee_id}`) || null;
    if (emp && emp.externalId && emp.externalId !== `hrhub:${r.employee_id}` && r.status !== 'W') {
      emp = null;
    }
    if (!emp && r.status === 'W') {
      emp =
    ...
    const empData = {
      ...
      externalId: `hrhub:${r.employee_id}`,
    };
  ...
  for (const r of locationsDump) {
    ...
    const payload = {
      ...
      isActive: true,
      meta: { hrhubLocationId: r.location_id, region: r.region_name, type: r.location_type_name },
    };
    if (locByName[name]) {
