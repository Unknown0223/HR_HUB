/**
 * Attach downloaded HR HUB JPEGs to FaceProfile (MinIO when available).
 * Photos: data/catalog-dump/live/photos/{hrhubEmployeeId}.jpg
 */
...
const PHOTOS = path.join(ROOT, 'data', 'hrhub-dump', 'live', 'photos');
...
(async () => {
  ...
  for (let i = 0; i < files.length; i += 1) {
    ...
    const emp = await prisma.employee.findFirst({
      where: { tenantId: tenant.id, externalId: `hrhub:${vfId}` },
      select: { id: true },
    });
    if (!emp) {
      miss += 1;
