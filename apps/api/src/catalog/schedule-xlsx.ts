...
export type CatalogScheduleDataRow = {
  id?: string;
  staff?: string;
  employee?: string;
  division?: string;
...
export type ParsedCatalogSchedule = {
  shifts: ScheduleShiftMeta[];
  rows: CatalogScheduleDataRow[];
  monthDays: number;
};
...
export async function buildIndividualScheduleTemplateBuffer(opts: {
  year: number;
  monthIndex: number;
  seedRows?: CatalogScheduleDataRow[];
  shifts?: ScheduleShiftMeta[];
}): Promise<Buffer> {
...
export async function parseIndividualScheduleWorkbook(
  ...
  const shifts: ScheduleShiftMeta[] = [];
  if (metaSheet) {
  ...
  const rows: CatalogScheduleDataRow[] = [];
  let monthDays = 31;
  if (dataSheet) {
