...
import { DynamicObjectFormModal } from './DynamicObjectFormModal';
...
export function DynamicObjectsPageInner({ kind }: { kind: 'entity' | 'fact' }) {
  ...
  const groupKey = kind === 'fact' ? 'dynamic-facts' : 'dynamic-objects';
  const title = kind === 'fact' ? 'Факты (метаданные)' : 'Объекты';
  const subtitle =
    kind === 'fact'
      ? 'Метаданные динамических фактов и связанных полей'
      : 'Динамические объекты и связанные с ними поля';
  const colLabel = kind === 'fact' ? 'Факт' : 'Объект';
  ...
  async function runDelete(row: DynamicObjectRow) {
    ...
    setError('');
    try {
      await apiFetch(`/api/catalog/dynamic-objects/${row.id}`, { method: 'DELETE' });
      setSelectedId(null);
      setChecked((prev) => {
        const next = { ...prev };
        delete next[row.id];
  ...
  async function runBulk(action: 'delete' | 'activate' | 'deactivate') {
    ...
    if (action === 'delete') {
      if (
        !(await confirm(
          `Удалить выбранные ${kind === 'fact' ? 'факты' : 'объекты'} (${targets.length} шт.)?`,
        ))
      ) {
        return;
      }
  ...
  function exportCsv() {
    downloadCsv(
      `${kind === 'fact' ? 'dynamic-facts' : 'dynamic-objects'}-${new Date().toISOString().slice(0, 10)}.csv`,
      filtered.map((r) => ({
        [colLabel]: r.name,
        Код: r.code,
  ...
  return (
    <div className={styles.wrap}>
      <PageSubnav groupKey={groupKey} />

      <div className={shared.pageHeader}>
        <div className={`${shared.pageIconBadge} ${shared.pageIconBadgeDoc}`}>
          <i
            className={kind === 'fact' ? 'fas fa-project-diagram' : 'fas fa-cube'}
            aria-hidden
          />
        </div>
