...
export function IndividualScheduleFormModal({
  open,
  editId,
  ...
  useEffect(() => {
    ...
    setLoading(true);
    apiFetch<DocRow>(`/api/catalog/schedule-overrides/${editId}`)
      .then((row) => {
  ...
  async function save(openDoc: boolean) {
    ...
    try {
      ...
      if (isEdit && editId) {
        await apiFetch(`/api/catalog/schedule-overrides/${editId}`, {
          method: 'PATCH',
  ...
  return (
    <FormModal
      open={open}
      title={
        isEdit ? 'Индивидуальный график (изменение)' : 'Индивидуальный график (создание)'
      }
      onClose={onClose}
      width="lg"
      footer={
      ...
      {loading ? (
        ...
        <div className={modal.fields}>
          ...
          <p className={styles.muted}>
            Строки сотрудников и сетка дней заполняются в документе — «Сохранить и
            заполнить».
          </p>
        </div>
      )}
