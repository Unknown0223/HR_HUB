...
const TITLE = 'Отчет по оплатам';
const FILE_BASE = 'Оплаты';
const MONTHS_LONG = [
...
export default function PaymentsReportPage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      setTree(divisions);
      const lookups = await apiFetch<{ employees?: Opt[] }>('/api/catalog/lookups').catch(() => ({ employees: [] as Opt[] }));
  ...
  const queryQs = useMemo(() => {
    ...
    return p.toString();
  }, [from, to, divisionIds, employeeIds]);

  const load = useCallback(async () => {
    setBusy(true);
    setError('');
    try {
      const data = await apiFetch<Payload>(`/api/catalog/analytics/payments?${queryQs}`);
      setReport(data);
      setLoadedQs(queryQs);
      return data;
    } catch (e) {
  ...
  async function exportExcel(data?: Payload | null) {
    ...
    if (!payload) return;
    const rows = [
      ...payload.rows.map((r) => [r.employee, r.position, r.division, money(r.cash), money(r.bank), money(r.total)]),
      ['Итого', '', '', money(payload.totals.cash), money(payload.totals.bank), money(payload.totals.total)],
    ];
    await downloadStyledXlsx({
      filename: `${FILE_BASE}(${fileStamp(payload.generatedAt)}).xlsx`,
      title: TITLE,
      subtitle: payload.periodLine,
      preamble: [payload.startDateLabel, payload.endDateLabel, ''],
      columns: ['Сотрудник', 'Должность', 'Подразделение', 'Наличные', 'Безналичные', 'Итого'],
      rows,
      colWidths: [36, 22, 24, 14, 14, 14],
    });
  }

  function csvText(data: Payload) {
    const q = (v: unknown) => `"${String(v ?? '').replace(/"/g, '""')}"`;
    const head = ['Сотрудник', 'Должность', 'Подразделение', 'Наличные', 'Безналичные', 'Итого'];
    const lines = [
      data.startDateLabel,
      data.endDateLabel,
      head.map(q).join(';'),
      ...data.rows.map((r) => [r.employee, r.position, r.division, money(r.cash), money(r.bank), money(r.total)].map(q).join(';')),
      ['Итого', '', '', money(data.totals.cash), money(data.totals.bank), money(data.totals.total)].map(q).join(';'),
    ];
    return `\ufeff${lines.join('\n')}`;
  }
  ...
  function xmlText(data: Payload) {
    const body = data.rows
      .map(
        (r) =>
          `<row employee="${escapeHtml(r.employee)}" position="${escapeHtml(r.position)}" division="${escapeHtml(r.division)}" cash="${r.cash}" bank="${r.bank}" total="${r.total}"/>`,
      )
      .join('');
    return `<?xml version="1.0" encoding="UTF-8"?><report title="${escapeHtml(TITLE)}" from="${data.from}" to="${data.to}" cash="${data.totals.cash}" bank="${data.totals.bank}" total="${data.totals.total}">${body}</report>`;
  }
  ...
  function printHtml(data: Payload) {
    const body = data.rows
      .map(
        (r) =>
          `<tr><td class="name">${escapeHtml(r.employee)}</td><td>${escapeHtml(r.position)}</td><td>${escapeHtml(r.division)}</td><td class="num">${money(r.cash)}</td><td class="num">${money(r.bank)}</td><td class="num">${money(r.total)}</td></tr>`,
      )
      .join('');
    const foot = `<tr class="total"><td class="name">Итого</td><td></td><td></td><td class="num">${money(data.totals.cash)}</td><td class="num">${money(data.totals.bank)}</td><td class="num">${money(data.totals.total)}</td></tr>`;
...
.wrap{padding:16px}
.meta{padding:0 16px;margin:0.25rem 0;font-size:13px}
...
<p class="meta">${escapeHtml(data.startDateLabel)}</p>
<p class="meta">${escapeHtml(data.endDateLabel)}</p>
<div class="wrap"><table>
<thead><tr><th>Сотрудник</th><th>Должность</th><th>Подразделение</th><th>Наличные</th><th>Безналичные</th><th>Итого</th></tr></thead>
    ...
    <div className={arena.page}>
      ...
      <div className={shared.pageHeader}>
        ...
        <div className={shared.pageHeaderText}>
          ...
          <p className={shared.pageSubtitle}>Оплаты сотрудникам наличными и безналичными за период</p>
        </div>
      </div>
                ...
                <table className={local.table}>
                  ...
                  <tbody>
                    {report.rows.length === 0 ? (
                      <tr>
                        <td className={local.empty} colSpan={6}>
                          Нет данных
                        </td>
                      </tr>
                    ) : (
                      report.rows.map((r) => (
                        <tr key={r.employeeId}>
                          ...
                          <td className={local.num}>{money(r.cash)}</td>
                          <td className={local.num}>{money(r.bank)}</td>
                          <td className={local.num}>{money(r.total)}</td>
                        </tr>
                      ))
                    )}
                    <tr className={local.totalRow}>
                      <td className={local.name}>Итого</td>
                      <td />
                      <td />
                      <td className={local.num}>{money(report.totals.cash)}</td>
                      <td className={local.num}>{money(report.totals.bank)}</td>
