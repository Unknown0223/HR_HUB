    ...
    <div className={`${arena.dropWrap}${menuOpen ? ` ${arena.dropOpen}` : ''}`} ref={wrapRef}>
      ...
      {selected.size ? (
        <div className={arena.chips}>
          <span className={arena.chip}>Выбрано: {selected.size}</span>
        </div>
      ) : null}
    </div>
  );
  ...
  return (
    <div className={`${arena.dropWrap}${open ? ` ${arena.dropOpen}` : ''}`} ref={wrapRef}>
      ...
      <div className={arena.dropPanel} hidden={!open}>
        <input
          className={arena.dropSearch}
          placeholder="Поиск..."
          value={q}
          onChange={(e) => setQ(e.target.value)}
        />
        {open && filtered.length === 0 ? <div className={arena.pickEmpty}>Нет данных</div> : null}
        {open
          ? filtered.map((o) => {
      ...
      {selected.length ? (
        <div className={arena.chips}>
          {selected.map((id) => {
            ...
            return (
              <button key={id} type="button" className={arena.chip} onClick={() => toggle(id)}>
                {label} ×
              </button>
            );
          })}
        ...
        {open ? (
          <>
            ...
            {filtered.map((o) => {
              const on = selected.includes(o.id);
              return (
                <button type="button" key={o.id} className={on ? `${arena.empRow} ${arena.empOn}` : arena.empRow} onClick={() => toggle(o.id)}>
                  <input type="checkbox" className={arena.box} readOnly checked={on} tabIndex={-1} />
...
<tbody>${body || `<tr><td colspan="${7 + extras.length + payload.days.length}">Нет данных</td></tr>`}</tbody>
  ...
  const exportBtns = (
    <div className={arena.exportBtns}>
      ...
      <button type="button" className={arena.exportBtn} disabled={busy} onClick={() => void ensureReport().then((d) => d && exportCsv(d))}>CSV</button>
      <button type="button" className={arena.exportBtn} disabled={busy} onClick={() => void ensureReport().then((d) => d && exportXml(d))}>XML</button>
    </div>
  );
      ...
      <div className={shared.pageHeader}>
        ...
        <div className={shared.pageHeaderText}>
          ...
          <p className={shared.pageSubtitle}>
            {isT13 ? 'Табель учета рабочего времени по форме Т-13: явки, отсутствия и итоги за период.' : 'Сводка явок, опозданий и отсутствий сотрудников по дням выбранного периода.'}
          </p>
        </div>
      </div>
            ...
            <>
              ...
              {settings.showColorDesc ? (
                <div className={arena.legend}>
                  <span><i className={arena.swatch} style={{ background: '#e7f3ff' }} />Выходной</span>
                  <span><i className={arena.swatch} style={{ background: '#fce4ec' }} />Отсутствие</span>
                  <span><i className={arena.swatch} style={{ background: '#fff2cc' }} />Опоздание / неполный день</span>
                </div>
              ) : null}
              <div className={local.tableWrap}>
                <table className={local.table}>
                  ...
                  <tbody>
                    {viewRows.length === 0 ? (
                      <tr>
                        <td className={local.empty} colSpan={7 + extras.length + metrics.length + report.days.length}>Нет данных</td>
                      </tr>
                    ) : (
                      viewRows.map((r) => (
