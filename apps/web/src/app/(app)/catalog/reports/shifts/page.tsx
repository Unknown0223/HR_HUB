    ...
    return (
      <>
        <div className={`${extra.treeRow} ${selected.has(node.id) ? arena.treeOn : ''}`} style={{ paddingLeft: depth * 14 }}>
          ...
          {kids.length ? <button type="button" className={arena.selectAll} onClick={() => selectBranch(node)}>выбрать все</button> : null}
        </div>
...
function ShiftsReportPageInner() {
  const searchParams = useSearchParams();
  const isV2 = searchParams.get('variant') === '2';
  const pageTitle = isV2 ? 'Отчет по сменам (второй вариант)' : TITLE;
  const pageSubtitle = isV2
    ? 'Альтернативный вариант отчёта посещений по сменам'
    : 'План, факт и отметки посещений сотрудников по сменам';
        ...
        <div className={arena.viewCard}>
          {busy && !report ? (
            ...
            <>
              ...
              {report.warnLine ? <p className={local.warn}>{report.warnLine}</p> : null}
...
export default function ShiftsReportPage() {
  return (
    <Suspense fallback={<p className={arena.muted}>Загрузка…</p>}>
      <ShiftsReportPageInner />
    </Suspense>
  );
}
