...
type AccrualKind = 'accrual' | 'deduction' | 'both';
...
const TITLE = 'Отчет по разовым начислениям';
const FILE_BASE = 'Отчет-по-разовым-начислениям';
const COLS = ['№', 'Сотрудник', 'Подразделение', 'Должность', 'Дата', 'Тип', 'Тип операции', 'Сумма', 'Примечание'] as const;
const MONTHS_LONG = [
...
export default function OneTimeAccrualsReportPage() {
  ...
  const [kind, setKind] = useState<AccrualKind>('both');
  ...
  const queryQs = useMemo(() => {
    ...
    return p.toString();
  }, [from, to, kind, divisionIds, positionIds, employeeIds]);

  const load = useCallback(async () => {
    setBusy(true);
    setError('');
    try {
      const data = await apiFetch<Payload>(`/api/catalog/analytics/one-time?${queryQs}`);
      setReport(data);
      setLoadedQs(queryQs);
      return data;
    } catch (e) {
  ...
  function rowCells(r: Row) {
    return [r.n, r.employee, r.division, r.position, r.date, r.type, r.operationType, money(r.amount), r.note];
  }
  ...
  async function exportExcel(data?: Payload | null) {
    ...
    await downloadStyledXlsx({
      ...
      preamble: [payload.periodLine, payload.typeLine, ''],
      columns: [...COLS],
      rows: payload.rows.map(rowCells),
      colWidths: [6, 36, 22, 22, 12, 20, 14, 14, 24],
    });
  }
  ...
  function csvText(data: Payload) {
    ...
    const lines = [
      ...
      data.typeLine,
      COLS.map(q).join(';'),
      ...data.rows.map((r) => rowCells(r).map(q).join(';')),
    ];
    return `\ufeff${lines.join('\n')}`;
  }
  ...
  function xmlText(data: Payload) {
    const body = data.rows
      .map(
        (r) =>
          `<row n="${r.n}" employee="${escapeHtml(r.employee)}" division="${escapeHtml(r.division)}" position="${escapeHtml(r.position)}" date="${escapeHtml(r.date)}" type="${escapeHtml(r.type)}" operationType="${escapeHtml(r.operationType)}" amount="${r.amount}" note="${escapeHtml(r.note)}"/>`,
      )
      .join('');
    return `<?xml version="1.0" encoding="UTF-8"?><report title="${escapeHtml(TITLE)}" from="${data.from}" to="${data.to}" kind="${data.kind}" amount="${data.totals.amount}" count="${data.totals.count}">${body}</report>`;
  }
  ...
  function printHtml(data: Payload) {
    ...
    const body = data.rows
      .map((r) => {
        ...
        return `<tr>${cells
          .map((c, i) => {
            const cls = i === 1 || i === 2 || i === 3 ? 'name' : i === 7 ? 'num' : i === 8 ? 'note' : '';
...
.note{text-align:left;white-space:normal}
...
<p class="meta">${escapeHtml(data.typeLine)}</p>
    ...
    <div className={arena.page}>
      ...
      <div className={shared.pageHeader}>
        ...
        <div className={shared.pageHeaderText}>
          ...
          <p className={shared.pageSubtitle}>Разовые начисления и удержания</p>
        </div>
      </div>
      ...
      {tab === 'filter' ? (
        <form className={`${arena.settingsCard} ${local.card}`} onSubmit={(e) => void generate(e)}>
          ...
          <div className={arena.field}>
            <label>Тип</label>
            <div className={local.radios}>
              <label>
                <input type="radio" name="ot-kind" checked={kind === 'accrual'} onChange={() => setKind('accrual')} />
                Начисление
              </label>
              <label>
                <input type="radio" name="ot-kind" checked={kind === 'deduction'} onChange={() => setKind('deduction')} />
                Удержание
              </label>
              <label>
                <input type="radio" name="ot-kind" checked={kind === 'both'} onChange={() => setKind('both')} />
                Оба
              </label>
            </div>
          </div>
                ...
                <table className={local.table}>
                  ...
                  <tbody>
                    {report.rows.length === 0 ? (
                      <tr>
                        <td className={local.empty} colSpan={9}>
                          Нет данных
                        </td>
                      </tr>
                    ) : (
                      report.rows.map((r) => (
                        <tr key={`${r.n}-${r.date}-${r.employee}-${r.type}`}>
                          <td>{r.n}</td>
                          <td className={local.name}>{r.employee}</td>
                          <td className={local.name}>{r.division}</td>
                          <td className={local.name}>{r.position}</td>
                          <td>{r.date}</td>
                          <td className={local.name}>{r.type}</td>
                          <td>{r.operationType}</td>
                          <td className={local.num}>{money(r.amount)}</td>
                          <td className={local.note}>{r.note}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
