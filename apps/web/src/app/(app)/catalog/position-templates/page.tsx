...
import { PositionTemplateFormModal } from './PositionTemplateForm';
...
const positionTemplateListPrefs = prefsConfigFromColumns({
  storageKey: 'hrhub.table.position-templates.v1',
...
function positionTemplateCell(row: PositionTemplateRow, key: string): string {
  switch (key) {
    case 'division':
...
function PositionTemplatesPageInner() {
  ...
  const prefs = useTablePrefs(positionTemplateListPrefs);
  const q = filters.q;
  ...
  const visibleCols = prefs.columns.length
    ? prefs.columns
    : positionTemplateListPrefs.defaultColumns;
  ...
  const displayRows = useMemo(
    () => prefs.applySortToRows(filtered, positionTemplateCell),
  ...
  async function runDelete(row: PositionTemplateRow) {
    if (!(await confirm('Удалить шаблон должности?'))) return;
    setBusy(true);
    setError('');
    try {
      await apiFetch(`/api/catalog/position-templates/${row.id}`, {
        method: 'DELETE',
      });
      setSelectedId(null);
      setChecked((prev) => {
  ...
  async function runBulk(action: 'delete' | 'activate' | 'deactivate') {
    ...
    if (action === 'delete') {
      if (
        !(await confirm(`Удалить выбранные шаблоны должностей (${targets.length} шт.)?`))
      ) {
        return;
      }
    }
  ...
  async function toggleActive(row: PositionTemplateRow, value: boolean) {
    setBusy(true);
    setError('');
    try {
      await apiFetch(`/api/catalog/position-templates/${row.id}`, {
  ...
  function exportCsv() {
    downloadCsv(
      ...
      displayRows.map((r) => {
        ...
        for (const k of visibleCols) obj[prefs.labelOf(k)] = positionTemplateCell(r, k);
        return obj;
      }),
    );
  }
  ...
  return (
    <div className={styles.wrap}>
      ...
      <PageSubnav groupKey="position-templates" />
      ...
      <div className={shared.pageHeader}>
        ...
        <div className={shared.pageHeaderText}>
          <h1 className={shared.pageTitle}>Шаблоны должностей</h1>
          <p className={shared.pageSubtitle}>
            Типовые связки подразделения, должности, разряда и графика
          </p>
        </div>
                    ...
                    >
                      <td className={styles.checkCol}>
                        <input
                          ...
                          aria-label={`Выбрать ${row.position?.name || row.code}`}
                        />
                      </td>
                      {visibleCols.map((key) => {
                        if (key === 'division') {
                        ...
                        return (
                          <td key={key}>{positionTemplateCell(row, key) || '—'}</td>
                        );
                      })}
                    </tr>
                    {open ? (
