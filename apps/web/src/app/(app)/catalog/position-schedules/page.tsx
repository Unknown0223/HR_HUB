...
function ListInner() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) =>
      [r.number, r.kind, KIND_LABEL[r.kind], r.division?.name, r.status, fmtMonth(r.month)]
        .filter(Boolean)
        .join(' ')
        .toLowerCase()
        .includes(q),
            ...
            {createOpen ? (
              <div className={styles.createMenu}>
                {KINDS.map((k) => (
                  <Link
                    key={k.kind}
                    href={`/catalog/position-schedules/new?kind=${k.kind}`}
                      ...
                      <td colSpan={7} style={{ padding: 0 }}>
                        <div className={styles.rowDetail}>
                          <Link
                            ...
                            href={`/catalog/position-schedules/${row.id}`}
                          >
                            Открыть
                          </Link>
                          {row.status === 'draft' ? (
