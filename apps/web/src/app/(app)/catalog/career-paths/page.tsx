...
function CareerPathsInner() {
  ...
  const filtered = useMemo(() => {
    ...
    if (qq) {
      list = list.filter((r) =>
        [r.name, r.code].filter(Boolean).join(' ').toLowerCase().includes(qq),
      );
    }
    return list;
  ...
  function applySearch() {
    ...
    router.replace(qs ? `/catalog/career-paths?${qs}` : '/catalog/career-paths', {
      scroll: false,
    });
  }
  ...
  async function runDelete(row: CareerPath) {
  ...
  async function runBulk(action: 'delete' | 'activate' | 'deactivate') {
    ...
    if (action === 'delete') {
      if (!(await confirm(`Удалить выбранные карьерные пути (${targets.length} шт.)?`)))
        return;
    }
      ...
      for (const row of targets) {
        try {
          if (action === 'delete') {
            ...
            await apiFetch(`/api/catalog/career-paths/${row.id}`, {
              method: 'PATCH',
  ...
  async function toggleActive(row: CareerPath, value: boolean) {
    setBusy(true);
    setError('');
    try {
      await apiFetch(`/api/catalog/career-paths/${row.id}`, {
    ...
    <div className={styles.wrap}>
      ...
      <div className={shared.pageHeader}>
        ...
        <div className={shared.pageHeaderText}>
          <h1 className={shared.pageTitle}>Карьерный путь</h1>
          <p className={shared.pageSubtitle}>
            Справочник карьерных путей и условий перехода между разрядами
          </p>
        </div>
                      ...
                      <tr className={styles.actionsRow}>
                        <td colSpan={colCount}>
                          <div className={styles.rowActions}>
                            ...
                            <Link href={`/catalog/career-paths/${row.id}`}>
