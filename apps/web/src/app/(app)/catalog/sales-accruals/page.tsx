...
function Inner() {
  ...
  async function bulk(kind: 'post' | 'unpost' | 'delete') {
    ...
    try {
      const ok = await runListBulk({
        path: `/api/payroll/sales-accruals/bulk-${kind}`,
        ids,
        message:
          kind === 'delete'
  ...
  async function run(row: SalesAccrualDoc, action: 'post' | 'unpost' | 'delete') {
    ...
    setError('');
    try {
      if (action === 'delete') await apiFetch(`/api/payroll/sales-accruals/${row.id}`, { method: 'DELETE' });
      else await apiFetch(`/api/payroll/sales-accruals/${row.id}/${action}`, { method: 'POST' });
      setFocusId(null);
      await load();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Ошибка');
