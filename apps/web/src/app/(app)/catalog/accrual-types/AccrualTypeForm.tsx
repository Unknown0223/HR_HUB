...
import type { AccrualTypeRow } from './page';
...
export function AccrualTypeForm({ mode, id }: Props) {
  const router = useRouter();
  const readOnly = mode === 'view';

  const [tab, setTab] = useState<'main' | 'tax' | 'reflect'>('main');
  ...
  const [taxNdfl, setTaxNdfl] = useState(true);
  const [taxInps, setTaxInps] = useState(false);
  const [taxOss, setTaxOss] = useState(false);
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const row = await apiFetch<AccrualTypeRow>(`/api/catalog/accrual-types/${id}`);
        if (cancelled) return;
        setName(row.name || '');
        ...
        setTaxNdfl(row.taxNdfl !== false);
        setTaxInps(Boolean(row.taxInps));
        setTaxOss(Boolean(row.taxOss));
  ...
  async function onSave(e: FormEvent) {
    ...
    try {
      if (mode === 'edit' && id) {
        await apiFetch(`/api/catalog/accrual-types/${id}`, {
          method: 'PATCH',
      ...
      router.push('/catalog/accrual-types');
      router.refresh();
    } catch (err) {
    ...
    <form className={styles.page} onSubmit={(e) => void onSave(e)}>
      <div className={styles.topBar}>
        ...
        <div className={styles.actions}>
          {!readOnly ? (
            ...
            <Link href={`/catalog/accrual-types/${id}/edit`} className={styles.btnSave}>
              Изменить
            </Link>
          )}
          <Link href="/catalog/accrual-types" className={styles.btnClose}>
            Закрыть
          </Link>
        </div>
      </div>
      ...
      <div className={styles.card}>
        ...
        <div className={styles.tabs}>
          ...
          <button
            type="button"
            className={tab === 'reflect' ? styles.tabOn : styles.tab}
          ...
          <div className={styles.mainGrid}>
            <div className={styles.col}>
              <div className={styles.field}>
                ...
                <input
                  list="accrual-purposes"
                  value={purpose}
                  onChange={(e) => setPurpose(e.target.value)}
                  disabled={readOnly}
                  placeholder="Поиск..."
                />
                <datalist id="accrual-purposes">
                  {PURPOSES.map((p) => (
              ...
              {resultMode === 'formula' ? (
                <div className={styles.field}>
                  ...
                  {!readOnly ? (
                    <button
                      type="button"
                      className={styles.formulaLink}
                      onClick={() => {
                        /* placeholder for formula builder */
                      }}
                    >
                      ✎ Изменить формулу
                    </button>
            ...
            <div className={styles.col}>
              <label className={styles.check}>
                <input
                  type="checkbox"
                  checked={taxNdfl}
                  onChange={(e) => setTaxNdfl(e.target.checked)}
                  disabled={readOnly}
                />
                Облагается НДФЛ
              </label>
              <label className={styles.check}>
                <input
                  type="checkbox"
                  checked={taxInps}
                  onChange={(e) => setTaxInps(e.target.checked)}
                  disabled={readOnly}
                />
                Удерживать взносы на ИНПС
              </label>
              <label className={styles.check}>
                <input
                  type="checkbox"
                  checked={taxOss}
                  onChange={(e) => setTaxOss(e.target.checked)}
                  disabled={readOnly}
                />
                Удерживать взносы на ОСС
              </label>
            </div>
            <div className={styles.col}>
              <fieldset className={styles.fieldset} disabled={readOnly}>
