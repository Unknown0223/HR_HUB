...
import { OneTimeFormModal } from './OneTimeFormModal';
...
function Inner() {
  ...
  async function runBulk(action: 'post' | 'unpost' | 'delete') {
    ...
    setError('');
    try {
      await apiFetch(`/api/payroll/one-time-accruals/bulk-${action}`, {
        method: 'POST',
  ...
  async function run(row: OneTimeDoc, action: 'post' | 'unpost' | 'delete') {
    ...
    try {
      if (action === 'delete') {
        await apiFetch(`/api/payroll/one-time-accruals/${row.id}`, { method: 'DELETE' });
      } else {
        await apiFetch(`/api/payroll/one-time-accruals/${row.id}/${action}`, { method: 'POST' });
      }
      setFocusId(null);
      setChecked((prev) => {
        const next = { ...prev };
    ...
    <div className={styles.wrap}>
      ...
      <div className={shared.pageHeader}>
        ...
        <div className={shared.pageHeaderText}>
          <h1 className={shared.pageTitle}>Разовые начисления / удержания</h1>
          <p className={shared.pageSubtitle}>Разовые документы начисления и удержания</p>
        </div>
      ...
      <div className={styles.tabs}>
        <Link href={`${PATH}?kind=accrual`} className={kind === 'accrual' ? styles.tabOn : styles.tab}>
          Начисление
        </Link>
        <Link
          href={`${PATH}?kind=deduction`}
          className={kind === 'deduction' ? styles.tabOn : styles.tab}
        >
          Удержание
        </Link>
      </div>
