...
type ColumnKey = 'name' | 'role' | 'group' | 'costAccount' | 'code';
...
const SETTINGS_KEY = 'hrhub_position_import_settings';

const FIELD_LABELS: { key: ColumnKey; label: string }[] = [
  { key: 'name', label: 'Должность' },
  { key: 'role', label: 'Роль' },
  { key: 'group', label: 'Группа должностей' },
  { key: 'costAccount', label: 'Счет затрат' },
  { key: 'code', label: 'Код' },
];
...
/** Matches Excel: Должность | Роль | Группа должностей | Счет затрат | Код */
  ...
  rows.forEach((row) => {
    if (!row.name.trim()) {
      errs.push({
        ...
        message: 'Должность (название) обязательна',
      });
    }
...
export default function PositionImportPage() {
  ...
  useEffect(() => {
    void (async () => {
      try {
        const rows = await apiFetch<{ code: string }[]>('/api/organization/positions');
        setExistingCodes(
          new Set(
  ...
  async function downloadTemplate() {
    ...
    try {
      await apiDownload(
        '/api/organization/positions/import/template.xlsx',
        'import-positions-template.xlsx',
      );
    } catch (e) {
  ...
  async function cancelStaging() {
    if (
      dirty &&
      !await confirm('Отменить виртуальные данные? Изменения не будут импортированы.')
    ) {
      return;
    }
    setFile(null);
  ...
  async function commitImport() {
    ...
    const blocking = errs.filter(
      (e) =>
        e.message.includes('обязательна') || e.message.includes('Дубликат кода'),
    );
    if (blocking.length) {
    ...
    try {
      ...
      const data = await apiFetch<ImportApiResult>('/api/organization/positions/import', {
        method: 'POST',
      ...
      if (data.created > 0 && (!data.errors || data.errors.length === 0)) {
        ...
        setFile(null);
        setChecked(false);
        const pos = await apiFetch<{ code: string }[]>('/api/organization/positions');
        setExistingCodes(
          new Set(
  ...
  const filteredStaging = useMemo(() => {
    ...
    return staging.filter((r) =>
      [r.name, r.code, r.role, r.group, r.costAccount].join(' ').toLowerCase().includes(q),
    );
  }, [staging, dataQ]);
  ...
  return (
    <div className={styles.wrap}>
      <PageSubnav groupKey="positions-import" />
      ...
      <div className={styles.toolbar}>
        ...
        <button
          ...
          onClick={async () => {
            if (
              dirty &&
              !await confirm('Есть несохранённые виртуальные данные. Закрыть?')
            ) {
              return;
            }
